...
const char** DivPlatformCGA1::getRegisterSheet() {
  return regCheatSheetCGA1;
}

void DivPlatformCGA1::rWrite(unsigned char addr, unsigned char val) {
...
int DivPlatformCGA1::dispatch(DivCommand c) {
  switch (c.cmd) {
    case DIV_CMD_NOTE_ON:
      ...
      chan[c.chan].macroInit(parent->getIns(chan[c.chan].ins,DIV_INS_CGA1));
    ...
    case DIV_CMD_PRE_PORTA:
      if (chan[c.chan].active && c.value2) {
        if (parent->song.compatFlags.resetMacroOnPorta) chan[c.chan].macroInit(parent->getIns(chan[c.chan].ins,DIV_INS_CGA1));
      }
...
DivMacroInt* DivPlatformCGA1::getChanMacroInt(int ch) {
  return &chan[ch].std;
}
...
unsigned char* DivPlatformCGA1::getRegisterPool() {
  return regPool;
}

int DivPlatformCGA1::getRegisterPoolSize() {
  return 128;
}
...
bool DivPlatformCGA1::keyOffAffectsArp(int ch) {
  return true;
}
...
void DivPlatformCGA1::poke(unsigned int addr, unsigned short val) {
  rWrite(addr,val);
}

void DivPlatformCGA1::poke(std::vector<DivRegWrite>& wlist) {
  for (DivRegWrite& i: wlist) rWrite(i.addr,i.val);
}

int DivPlatformCGA1::init(DivEngine* p, int channels, int sugRate, const DivConfig& flags) {
  parent=p;
  dumpWrites=false;
...
DivPlatformCGA1::~DivPlatformCGA1() {
}
